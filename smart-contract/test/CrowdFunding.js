...
describe("CrowdFunding Contract", () => {
  ...
  it("should donate to a campaign", async () => {
    await crowdFunding
      .connect(donor)
      .donate(campaignId, { value: ethers.parseEther("100") });
    const campaign = await crowdFunding.campaigns(campaignId);
  ...
  it("should withdraw from a campaign", async () => {
    await crowdFunding
      .connect(donor)
      .donate(campaignId, { value: ethers.parseEther("50") });
    await crowdFunding
      .connect(owner)
      .withdraw(campaignId, ethers.parseEther("30"));
    const campaign = await crowdFunding.campaigns(campaignId);
  ...
  it("should get donations for a campaign", async () => {
    ...
    await crowdFunding
      .connect(donor)
      .donate(campaignId, { value: ethers.parseEther("30") });
